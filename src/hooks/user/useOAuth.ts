...
export default function () {
     ...
     * @param pathname  
     */
    const to_login_by_qq = async ({ access_token, expired_time, type, pathname }: { access_token: string, expired_time: Date, type: string, pathname?: string }) => {
        try {
            //登录、绑定或注册
    ...
    const to_qq_oauth = async ({ is_oauth = false, isPC = true, type = 'login' }) => {
        ...
        const redirect_uri = encodeURIComponent(config.options_qq.redirectURI + (isPC ? '' : '&display=mobile&pathname=' + (route.name?.toString() || 'home')))
        ...
        const url = import.meta.env.DEV ? (import.meta.env.VITE_BASE_IP + `/oauth?reffer=qq&#access_token=BB3E00F21390900B9BD788D7B2A8A2DA&expires_in=5184000&state=${state}&display=${display}`) : oauth_url
