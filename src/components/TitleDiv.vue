<template>
  ...
  <!--登录和注册窗口-->
  <el-drawer v-model="drawer" class="login" :before-close="isClose" :show-close="false"
...
onMounted(() => {
  ...
  //修改dialog框的宽度
  if (width <= 980) dialogWidth.value = '90%'
  else dialogWidth.value = '60%'
  //如果本地壁纸为空
  if (bgUrl.value === null || bgUrl.value === '') {
    //响应式布局,判断是手机则用手机背景图
    if (width <= 980) changeBG(1)
    else changeBG(0)
  } else {
