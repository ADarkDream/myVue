  ...
  <el-container @touchend="stopTouch" @touchstart="stopTouch">
    <el-header>
      <el-breadcrumb :separator-icon="ArrowRight">
        <el-breadcrumb-item :to="{ name: 'books' }"
          >账本列表</el-breadcrumb-item
        >
      ...
      <el-tabs v-model="activeName" :tab-position="tabPosition">
        <el-tab-pane label="账本月历" :name="0" :style="tabContentHeight">
          <BookCalendar
            ...
            :get-the-bill-desc="getTheBillDesc"
          />
        </el-tab-pane>
        <el-tab-pane label="账单详情" :name="1">
          <BillDetails
            :delete-row="deleteRow"
            :is-null="bookInfo.isNull"
            :show-bill-panel="showBillPanel"
          />
        </el-tab-pane>
        ...
        <el-tab-pane label="协作成员" :name="3">
          ...
          <div v-else>
            <div style="display: flex; justify-content: space-evenly">
              <el-text type="primary" v-for="item in members" :key="item.uid">{{
                item.username
              }}</el-text>
            </div>
            <el-button
...
const tabContentHeight = ref(
  isPC.value
    ? ""
    : "height:" + (screenHeight.value - 145) + "px;overflow:scroll"
);
...
const getTheBookDesc = async () => {
  try {
    const result = await momo.get<{ book: Book; members: Member[] }>(
      "/theBookDesc",
      {
...
const getTheBillDesc = async (start_date: string, end_date: string) => {
  try {
    const result = await momo.get<{ billsDesc: Record<string, BillDesc> }>(
      "/theBillDesc",
      {
        bid: bookInfo.value.bid,
        start_date,
...
const getTheBook = async (start_date: string, end_date: string) => {
  try {
    ...
    if (data!.bills.length === 0)
      return ElMessage.info("当前时间范围内不存在账单");
    else computation();
  } catch (error) {
    console.log("发生错误：");
    console.dir(error);
...
const computation = () => {
  ...
  bookData.billList.forEach((bill) => {
    ...
    if (status === 0 || status === 1) {
      //计算每个用户的总花费
      memberInfo[uid].totalExpense = numPlus(
        memberInfo[uid].totalExpense,
        price
      );
      //计算所有人的总花费
