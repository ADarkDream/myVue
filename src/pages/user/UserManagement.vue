<template>
  ...
  <el-main v-if="tableVisible" style="padding: 0">
    ...
    <template v-if="draftVisible">
      ...
      <!--    表格模式；文章-->
      <el-table ref="articleRef" :data="allArticleList" max-height="500" stripe border highlight-current-row
        ...
        <el-table-column prop="tags" label="标签" :filters="[
          { text: '标签b', value: '标签b' },
          { text: '散文', value: '散文' },
        ]" :filter-method="filterHandler">
          <template #default="scope">
        ...
        <el-table-column fixed="right" label="操作" min-width="140">
          <template #default="scope">
            <el-button v-if="scope.row.status === 1" link type="primary" size="small"
              @click="goArticle(scope.row, 1)">查看
            </el-button>
            <el-button v-else link type="primary" size="small" @click="goArticle(scope.row, 0)">查看
            </el-button>
            <el-button link type="primary" size="small" @click="goEdit(scope.row, 0)">编辑
            </el-button>
            <el-button link type="danger" size="small" @click="deleteRow(scope.row.id, 0)">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <template v-else>
      ...
      <el-table ref="draftRef" :data="draftList" max-height="500" stripe border highlight-current-row
        style="width: 100%" table-layout="auto" :default-sort="{ prop: 'id', order: 'descending' }">
        <el-table-column fixed prop="id" label="ID" sortable />
        <el-table-column prop="title" label="草稿标题" />
        <el-table-column prop="area" label="板块" :filters="[{ text: '文章', value: '文章' }, { text: '板块一', value: '板块一' },]"
          :filter-method="filterHandler">
          <template #default="scope">
            <div v-if="scope.row.area == '文章'">文章</div>
            <div v-else-if="scope.row.area === '板块一'">板块一</div>
        ...
        <el-table-column prop="tags" label="标签" :filters="[{ text: '标签b', value: '标签b' }, { text: '散文', value: '散文' },]"
          :filter-method="filterHandler">
          <template #default="scope">
            <div v-if="scope.row.tags == '标签b'">标签b</div>
            <div v-else-if="scope.row.tags === '散文'">散文</div>
    ...
    <div class="div">
      <el-container>
        <el-main class="cards">
          <el-button text type="info" class="title">草稿箱</el-button>
          <el-divider>{{ draftList.length }}</el-divider>
          <el-card v-for="item in draftList" class="card">
...
const goEdit = (item: Article, isDraft: number) => {
  ...
  router.push({ name: 'userEdit', query: { id, title, area, tags, html, isDraft } })
}
