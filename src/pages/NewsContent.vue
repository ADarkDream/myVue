      ...
      <el-container style="min-height: 400px">
        ...
        <el-footer>
          <el-space class="articleFooter" style="text-align: initial; font-size: 14px">
            <span
              >板块：<el-button link @click="go('', article.area, '')" type="primary"> {{ article.area }}</el-button
              ><br />
              标签：<el-button link @click="go('', '', article.tags)" type="primary"> {{ article.tags }}</el-button></span
            >
            <span
              >发布于：{{ getDiffTime(article.created_time) }}<br /><template v-if="article.created_time !== article.updated_time"
                >修改于：{{ getDiffTime(article.updated_time) }}</template
              ></span
            >
          </el-space>
        </el-footer>
