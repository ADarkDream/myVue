  ...
  <div class="MusicListDrawer">
    ...
    <div v-for="(item, index) in myMusicListInfo" :key="index">
      <div class="musicDiv" @click="connectMusicToList(item.music_list_id)">
        <div class="img" :style="'--coverImage:' + `url(${item.pic_url || item.default_cover_url || defaultAlbumArt})`"></div>
        <div>
          <p>
            <el-text>{{ item.name }}——共{{ item.songsCount }}首</el-text>
          </p>

          <el-text :type="item.status === 1 ? 'primary' : 'success'">{{ item.status === 1 ? "私有" : "公开" }}</el-text>
