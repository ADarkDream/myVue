<template>
  <div class="musicAdd">
    <div class="bar">
      <el-button class="button" @click="ElMessage.info('功能开发中')"
        ><el-icon> <Upload /> </el-icon>上传</el-button
      >
      <el-button
        class="button"
        @click="musicSearchStore.isShowSearchPanel = true"
        ><el-icon> <Search /> </el-icon>搜索网易云音乐曲库</el-button
      >
      <el-button class="button" @click="getCustomMusicList()"
    ...
    >
      <el-form-item>
        <template #label
          ...
          <Transition name="horizontal_slide">
            <el-text v-show="true"> 用户自行上传的歌曲不可见 </el-text>
          </Transition>
        </template>
        <el-row :gutter="24">
          <el-col :span="18">
            <el-input
              v-model.trim.number="cloudMusicListID"
              placeholder="输入网易云歌单ID或分享链接"
            />
          </el-col>
          <el-col :span="6">
            <el-button
              plain
              type="success"
              @click="search_song_or_list(cloudMusicListID, false)"
            >
              搜索
            </el-button>
          </el-col>
      ...
      <el-form-item label="输入音乐ID获取音频链接(仅支持网易云免费音乐)">
        <el-row :gutter="24">
          <el-col :span="18">
            <el-input
              v-model.trim.number="cloudMusicID"
              placeholder="输入网易云音乐ID或分享链接"
            ></el-input>
          </el-col>
          <el-col :span="6">
            <el-button
              plain
              type="primary"
              @click="search_song_or_list(cloudMusicID, true)"
              >添加并播放
            </el-button>
          </el-col>
        </el-row>
      ...
      <el-form-item label="输入音频链接" prop="src">
        <el-row :gutter="24">
          ...
          <el-col :span="6">
            <el-button plain type="primary" @click="addMusic(musicUrl)"
              >添加并播放</el-button
            >
          </el-col>
        </el-row>
