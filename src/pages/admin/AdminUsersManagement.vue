<template>
  <el-container>
    ...
    <el-main style="padding-bottom: 0; padding-top: 0">
      ...
      >
        <el-table-column fixed label="UID" prop="uid" sortable width="80" />
        <el-table-column label="用户昵称" prop="username" width="200">
        ...
        <el-table-column label="个性签名" prop="signature" width="250">
          <template #default="scope">
            <div v-if="isEditRow === scope.$index">
              <el-input
        ...
        >
          <template #default="scope">
            ...
            <!--            </el-select>-->
            <!--          </div>-->
            <el-button v-if="scope.row.headImgStatus === 0" text type="primary"
              >待审核</el-button
            >
            <el-button
              v-else-if="scope.row.headImgStatus === 1"
              text
              type="info"
              >审核通过</el-button
            >
            <el-button
              v-else-if="scope.row.headImgStatus === 2"
              text
              type="danger"
              >黑名单</el-button
            >
        ...
        <el-table-column align="center" label="背景图" prop="bgUrl" width="150">
          <template #default="scope">
            <div v-if="isEditRow === scope.$index">
        ...
        >
          <template #default="scope">
            ...
            <!--            </el-select>-->
            <!--          </div>-->
            <el-button v-if="scope.row.bgStatus === 0" text type="primary"
              >待审核</el-button
            >
            <el-button v-else-if="scope.row.bgStatus === 1" text type="info"
              >审核通过</el-button
            >
            <el-button v-else-if="scope.row.bgStatus === 2" text type="danger"
              >黑名单</el-button
            >
          </template>
        </el-table-column>
        ...
        <el-table-column label="更新时间" prop="updated_time" width="150">
          <template #default="scope">{{
            getTime(scope.row.updated_time)
          }}</template>
        </el-table-column>
        <el-table-column v-if="isSuperAdmin" label="设为管理员" width="100">
          <template #default="scope">
            <el-button
              link
              size="small"
              type="primary"
              @click="addAdmin(scope.row.uid)"
              >设为管理员</el-button
            >
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            <div v-if="isEditRow !== scope.$index">
              ...
              <el-button
                ...
                size="small"
                type="danger"
                @click="deleteRow(scope.$index, scope.row, false)"
              >
                删除
              </el-button>
            </div>
            <div v-else>
              ...
              <el-button
                ...
                size="small"
                type="primary"
                @click="checkUpdateRow(newUserInfo, scope.row, false)"
              >
                更新
              </el-button>
            </div>
      ...
      >
        <el-table-column fixed label="UID" prop="uid" sortable width="80" />
        <el-table-column label="管理员昵称" prop="username" width="200">
          <template #default="scope">
            <div v-if="isEditRow === scope.$index">
              <el-input
                v-model="newUserInfo.username"
        ...
        <el-table-column label="管理员邮箱" prop="email" width="200">
          <template #default="scope">
            <div v-if="isEditRow === scope.$index">
              <el-input v-model="newUserInfo.email" />
            </div>
        ...
        <el-table-column label="管理员密码" prop="password" width="250">
          <template #default="scope">
            <div v-if="isEditRow === scope.$index">
              <el-input
            ...
            <el-button v-else-if="scope.row.password === ''" text type="info"
              >不做展示</el-button
            >
          </template>
        </el-table-column>
        ...
        >
          <template #default="scope">
            <div v-if="isEditRow === scope.$index">
              <el-select
                v-model="newUserInfo.isSuperAdmin"
                default-first-option
                placeholder="选择状态(默认为普通管理员)"
              >
            ...
            <el-button v-else-if="scope.row.isSuperAdmin === 0" text type="info"
              >普通管理员</el-button
            >
            <el-button
              v-else-if="scope.row.isSuperAdmin === 1"
              text
              type="primary"
              >超级管理员</el-button
            >
            <el-button
              v-else-if="scope.row.isSuperAdmin === 2"
              text
              type="danger"
              >权限冻结</el-button
            >
          ...
          <template #default="scope">
            <div v-if="isEditRow !== scope.$index">
              ...
              <el-button
                ...
                size="small"
                type="danger"
                @click="deleteRow(scope.$index, scope.row, true)"
              >
                删除
              </el-button>
            </div>
            <div v-else>
              ...
              <el-button
                ...
                size="small"
                type="primary"
                @click="checkUpdateRow(newUserInfo, scope.row, true)"
              >
                更新
              </el-button>
            </div>
...
function checkUpdateRow(
  ...
  if (Object.keys(data).length === 0)
    return ElMessage.info("用户信息未修改，已取消上传。");
