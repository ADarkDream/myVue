    ...
    >
      ...
      >
        <template #default="scope">
          ...
          <el-button v-else-if="scope.row.status === 0" text type="danger"
            >未过审</el-button
          >
          <el-button v-else-if="scope.row.status === 2" text type="primary"
            >待审核</el-button
          >
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" fixed="right" label="操作" width="150">
        <template #default="scope">
          ...
          <div v-else>
            <el-button link size="small" type="primary" @click="isEditRow = -1"
              >取消
            </el-button>
            <el-button
              link
              size="small"
              type="primary"
              @click.prevent="checkUpdateRow(newComment, scope.row)"
            >
              更新
            </el-button>
          </div>
...
function checkUpdateRow(newData: Comment, oldData: Comment) {
  ...
  if (Object.keys(data).length === 0)
    return ElMessage.info("评论信息未修改，已取消上传。"); //校验格式
