    ...
    >
      ...
      >
        <template #default="scope">
          ...
          <el-button
            v-else-if="scope.row.sort === 'blacklist' || scope.row.status === 2"
            text
            type="danger"
            >黑名单
          </el-button>
          <el-button v-else-if="scope.row.sort === 'headImg'" text
            >头像</el-button
          >
      ...
      >
        <template #default="scope">
          <div v-if="isEditRow === scope.$index">
            <el-select
              v-model="urlInfo.status"
              default-first-option
              placeholder="选择状态(默认为待审核)"
            >
        ...
        <template #default="scope">
          ...
          <div v-else>
            ...
            <el-button
              ...
              size="small"
              type="primary"
              @click.prevent="checkUpdateRow(urlInfo, scope.row)"
            >
              更新
            </el-button>
          </div>
...
function checkUpdateRow(newData: Image, oldData: Image) {
  ...
  if (Object.keys(data).length === 0)
    return ElMessage.info("图片信息未修改，已取消上传。");
