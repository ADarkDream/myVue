  ...
  <div ref="download1999" class="download1999">
    <div style="position: relative">
      ...
      <el-collapse v-model="activeIndex" accordion>
        ...
        <el-collapse-item name="2" title="筛选条件【所有条件不选则默认全选】">
          <el-form :label-position="isPC ? 'left' : 'top'">
            ...
            <div style="text-align: center">
              ...
              <el-button :icon="Search" type="primary" @click="getImages"
                >筛选</el-button
              >
              <el-button
                v-show="isShow"
              ...
              <el-button
                v-show="isShow"
                :icon="chooseType[isChoose].icon"
                :type="chooseType[isChoose].type"
                @click="selectBtn()"
              >
                <span>{{ chooseType[isChoose].text }}</span>
              </el-button>
              <el-button
                v-if="imgList.length && isPC"
                type="success"
                @click="checkPort()"
                >检查本地代理</el-button
              >
              ...
              <div class="statement">
                ...
                <el-text
                  type="success"
                  @click="showNotice({ show_num: 3, active_num: 1 })"
        ...
        <el-collapse-item name="3" title="待完善功能">
          <el-text
            v-for="item in unCompleted.slice().reverse()"
            :key="item.id"
            tag="p"
          >
        ...
        <el-collapse-item name="4" title="已实现功能">
          <el-text
            v-for="(item, index) in completed.slice().reverse()"
            :key="index"
            tag="p"
          >
        ...
        >
          ...
          <el-text
            title="点击反馈"
            type="success"
            @click="showNotice({ show_num: 3, active_num: 2 })"
            >反馈
          </el-text>
          ...
          <el-button
            ...
            type="primary"
            @click="
              copyText('904688184', 'QQ群号', 'https://qm.qq.com/q/Oq8R7YS6sM')
            "
          >
            点击加入群聊【金兔子特供部门🐰】
          </el-button>
          ...
          <br />
          赞赏者名单：
          <el-text>(设计中)</el-text>&ensp; 当前收到赞赏：
          <el-text type="success">{{ fee }}</el-text
          >&ensp;元(手动录入会有延迟)
          <br />
          <el-button
            v-if="!showPayCode"
    ...
    <el-affix :offset="isPC ? 50 : 85" position="top" target=".download1999">
      <div v-show="isShow" class="floatBar">
        <el-button :icon="Top" type="default" @click="scrollToTop"
          >返回顶部</el-button
        >
    ...
    >
      ...
      <DownloadNotice
        :download-limit-num="downloadLimitNum"
        :show-flag="showFlag"
        :show-pay-code-panel="showPayCodePanel"
      />
    </el-dialog>
  </div>
</template>
...
const {
  diffVersionInfo: versionInfo,
  diffRoleInfo: roleInfo,
  campInfo,
  raceInfo,
} = toRefs(reverse1999Store);
...
const pay_code_src = ref(
  import.meta.env.VITE_QINIUTHUMBNAIL_URL + "/files/payCode.png"
);
...
onMounted(async () => {
  ...
  ElMessage.warning(
    "如果页面出错请刷新两下，刷新后还没有可能是网站在升级，请稍后访问"
  );
});
...
const handleCheckAllVersionChange = (val: boolean) => {
  if (val)
    versionInfo.value.forEach((item) => condition.version.push(item.version));
...
const handleCheckedVersionsChange = () => {
  ...
  isIndeterminateVersion.value =
    checkedCount > 0 && checkedCount < versionInfo.value.length; //未全选时变更按钮为 -
};
...
const handleCheckedRolesChange = () => {
  ...
  isIndeterminateRole.value =
    checkedCount > 0 && checkedCount < roleInfo.value.length; //未全选时变更按钮为 -
...
const handleCheckCampChange = (val: boolean) => {
  ...
  roleInfo.value.forEach((item) => {
    ...
    if (typeFlag.value === 1) {
      //筛选阵营
      if (item.camp === campName.value)
        val ? newList.add(item.id) : newList.delete(item.id);
    } else if (typeFlag.value === 2) {
      //筛选种族
      if (item.race === raceName.value)
        val ? newList.add(item.id) : newList.delete(item.id);
    }
  });
...
const getImages = async () => {
  ...
  //如果全选版本，则直接全部清除
  if (condition.version.length === versionInfo.value.length)
    condition.version.splice(0, condition.version.length);
  ...
  //判断筛选条件是否改变
  if (deepEqual(condition, oldCondition.value, true))
    return ElMessage.info("筛选条件未作改变，已取消查询");
  // else {
  // 将 a 的值同步到 b，包括空值
...
function checkImage(imgInfo: ReverseImg, index: number, e: Event) {
  ...
  if (isChoose.value === "none") {
    ...
    if (
      target.tagName !== "IMG" ||
      target.classList.contains("el-image-viewer__img")
    )
      return; //如果点击的不是图片元素则终止函数,以防重复添加
  ...
  if (length <= downloadLimitNum.value) {
    ...
    if (!isPC.value)
      await ElMessageBox.confirm("移动端浏览器可能无法批量下载，是否继续？", {
        ...
        .then(() =>
          ElMessage.info("如等待之后没有下载，请更换浏览器或长按图片保存")
        )
        .catch(() => (flag = false));
    if (!flag) return;
    console.log("下载列表：", downloadList);
    downloadList.value.forEach((item) =>
      downloadImg(item.imgUrl, item.newName, item.imgPath!)
    );
    selectBtn("part");
  } else {
    //下载数量大于5
    ...
    if (isOpenProxy.value) {
      ...
      return ElMessage.error(
        "当前下载数量大于10且未开启代理，请先查看下载须知→下载大量"
      );
  }
};
...
const downloadImg = async (url: string, imgName: string, imgPath: string) => {
  ...
  if (isOpenProxy.value)
    ...
    imageUrl = url.replace(
      ...
      "http://localhost:3000/download1999"
    );
  else if (imgPath) {
    //没有端口代理
    ...
    //没有端口代理且服务器没有备份
    imageUrl = url.replace(
      axios.defaults.baseURL + "/download1999",
      "http://localhost:3000/download1999"
    );
  }
  try {
...
function autoCol(num?: number) {
  ...
  if (previewImgList.value.length <= 15 && isPC.value) {
    ...
    console.log(
      previewImgList.value.length + "张,少于15张,计算的图片列数:" + colNum.value
    );
  } else {
