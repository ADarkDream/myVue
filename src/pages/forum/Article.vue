        ...
        <div style="margin-bottom: 30px">
          ...
          <el-row v-else v-for="(item, index) in comments" :key="index" class="comments" :gutter="10">
            ...
            <el-col :sm="22" :xs="20" style="display: flex; flex-wrap: wrap"
              ...
              <!--评论区顶部菜单-->
              <el-text class="commentsContent">&ensp; &ensp; {{ item.comment }}</el-text
              ><!--评论内容-->
            </el-col>
          </el-row>
        </div>
...
onMounted(async () => {
  ...
  await nextTick(() => {
    ...
    if (comments.value.length === 0) console.log("没有评论")
      ...
      //有评论时获取输入评论框距离页面顶部的距离
      if (commentBox.value) commentsBoxOffsetTop.value = commentBox.value.offsetTop
    }
  })
})
//endregion
