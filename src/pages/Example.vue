        ...
        <div class="control">
            <el-form :model="resortRule" :inline="true">
                <el-form-item prop="star" label="星级筛选" :class="{ 'checked': !!resortRule.star }">
                    <el-select v-model="resortRule.star" placeholder="选择星级" clearable filterable
                        @change="filterAndSortRoles" multiple>
                        <el-option v-for="item in options_star" :key="item.value" :label="item.label"
                            :value="item.value">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item prop="damageType" label="伤害类型筛选" :class="{ 'checked': !!resortRule.damageType }">
                    <el-select v-model="resortRule.damageType" placeholder="选择伤害类型" clearable default-first-option
                        filterable @change="filterAndSortRoles">
                        <el-option v-for="item in options_damageType" :key="item.value" :label="item.label"
                            :value="item.value">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item prop="valueType" label="排序类型">
                    <el-select v-model="resortRule.valueType" placeholder="选择伤害类型" clearable default-first-option
                        filterable @change="filterAndSortRoles">
                        <el-option v-for="item in options_valueType" :key="item.value" :label="item.label"
                            :value="item.value">
                        </el-option>
                    </el-select>
                </el-form-item prop="isAsc" label="排序方向">
                <el-switch v-model="resortRule.isAsc" :active-value="true" active-text="升序" :inactive-value="false"
                    inactive-text="降序" @change="filterAndSortRoles">
                </el-switch>
            </el-form>
...
function filterAndSortRoles() {
    showList.value = roles
        ...
        .sort((a: any, b: any) => {
            ...
            return resortRule.isAsc ? a[resortRule.valueType] - b[resortRule.valueType] : b[resortRule.valueType] - a[resortRule.valueType]; // 数值降序
        });
}
