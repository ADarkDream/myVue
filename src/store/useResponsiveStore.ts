...
export const useResponsiveStore = defineStore(
  ...
  () => {
    ...
    const isDark = ref<boolean>(
      sessionStorage.getItem("isDark") === "1" || window.matchMedia("(prefers-color-scheme: dark)").matches || false
    )
    ...
    const positionComputed = (e: TouchEvent, activeName: Ref<number>, minNum = 0, maxNum: number) => {
      ...
      switch (flag) {
        ...
        case 3: {
          ...
          if (activeName.value < maxNum) activeName.value++
    ...
    function checkRoute(path: string) {
      ...
      //初始不显示，显示之后就不再判断
      if (!showPlayer.value) showPlayer.value = path.includes("/music")
    }
