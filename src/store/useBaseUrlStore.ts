...
export const useBaseUrlStore = defineStore('baseUrl', () => {
    ...
    //聊天服务器基地址
    const chatBaseUrl = import.meta.env.DEV === true ? 'http://127.0.0.1:9999/' : 'https://muxidream.cn/'

    //七牛云存储基地址
    const qiniuHttpUrl = 'http://qiniufree.muxidream.cn'
    const qiniuHttpsUrl = 'https://qiniu.muxidream.cn'  //HTTPS收费，需要去七牛云控制台开启，域名控制台开启解析
   //根据本地环境自动更换七牛url前缀
    const defaultUrl = import.meta.env.DEV === true ? 'http://qiniufree.muxidream.cn' : 'https://qiniu.muxidream.cn'
    return {chatBaseUrl, qiniuHttpUrl, qiniuHttpsUrl,defaultUrl}
})
