...
export const useVersionsStore = defineStore("1999_versions", () => {
  ...
  const handleChange = (currentVersion = formData.value.version) => {
    const info = versionInfo.value.find(
    ...
    if (info && isEdit.value) {
      ...
      (formData.value.time = time || []), (formData.value.status = status || 1);
    ...
    formData.value.halfText = currentVersion
      ? "V" + (currentVersion / 10).toFixed(1) + "_"
      : "";
  };
  ...
  return {
    ...
    isEdit,
    isDisabled,
    isShowAddVersionDrawer,
    oldFormData,
    formData,
    reSetFormData,
    toggleAddVersionDrawer,
    handleChange,
  };
});
